...
DROP MATERIALIZED VIEW IF EXISTS irs_sop_avg_time CASCADE;

CREATE MATERIALIZED VIEW IF NOT EXISTS irs_sop_avg_time
AS
SELECT
    public.uuid_generate_v5('6ba7b810-9dad-11d1-80b4-00c04fd430c8'::uuid, concat(subq.district_id, subq.plan_id, subq.data_collector, subq.sop)) AS id,
    subq.plan_id,
    subq.district_id,
    subq.data_collector,
    subq.sop,
...
FROM (
    ...
    GROUP BY irs_spray_event.event_date, irs_spray_event.plan_id, irs_spray_event.district_id, irs_spray_event.data_collector, irs_spray_event.sop) subq

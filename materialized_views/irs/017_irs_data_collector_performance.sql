...
DROP MATERIALIZED VIEW IF EXISTS irs_data_collector_performance CASCADE;

CREATE MATERIALIZED VIEW IF NOT EXISTS irs_data_collector_performance
AS
SELECT
    subq.id,
    subq.plan_id,
...
FROM
    (
        ( WITH aggregates AS (
            SELECT
                public.uuid_generate_v5('6ba7b810-9dad-11d1-80b4-00c04fd430c8'::uuid, concat(spray_event.district_id, spray_event.plan_id, spray_event.data_collector)) AS id,
                    spray_event.plan_id,
                    ...
                    COALESCE(sum(spray_event.other_reason), (0)::bigint) AS other_reason,
                ...
                GROUP BY spray_event.data_collector, spray_event.plan_id, spray_event.district_id
            )
        SELECT
            aggregates.id,
            aggregates.plan_id,
        ...
        LEFT JOIN irs_collector_avg_time avg_time ON ((((avg_time.plan_id)::text = (subq.plan_id)::text) AND ((avg_time.district_id)::text = (subq.district_id)::text) AND ((avg_time.data_collector)::text = (subq.data_collector)::text))));

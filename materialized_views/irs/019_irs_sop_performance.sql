...
DROP MATERIALIZED VIEW IF EXISTS irs_sop_performance CASCADE;

CREATE MATERIALIZED VIEW IF NOT EXISTS irs_sop_performance
AS
SELECT
    subq.id,
    subq.sop,
    ...
    FROM (( WITH aggregates AS (
                  SELECT public.uuid_generate_v5('6ba7b810-9dad-11d1-80b4-00c04fd430c8'::uuid, concat(spray_event.district_id, spray_event.plan_id, spray_event.data_collector, spray_event.sop)) AS id,
                     spray_event.sop,
                     spray_event.plan_id,
                     spray_event.district_id,
                     spray_event.data_collector,
                    ...
                    FROM (irs_spray_event spray_event
                      JOIN daily_summary_event daily_summary ON ((((daily_summary.location_id)::text = (spray_event.location_id)::text) AND ((daily_summary.plan_id)::text = (spray_event.plan_id)::text) AND (daily_summary.collection_date = spray_event.event_date) AND ((daily_summary.provider_id)::text = (spray_event.data_collector)::text) AND (daily_summary.sop = spray_event.sop))))
                   GROUP BY spray_event.sop, spray_event.plan_id, spray_event.district_id, spray_event.data_collector
                 )
          SELECT aggregates.id,
             aggregates.sop,
             aggregates.plan_id,
             aggregates.district_id,
             aggregates.data_collector,
             aggregates.found,
             aggregates.sprayed,
             aggregates.refused,
            ...
            FROM aggregates) subq
      LEFT JOIN irs_sop_avg_time avg_time ON ((((avg_time.plan_id)::text = (subq.plan_id)::text) AND ((avg_time.district_id)::text = (subq.district_id)::text) AND ((avg_time.data_collector)::text = (subq.data_collector)::text) AND (avg_time.sop = subq.sop))));
